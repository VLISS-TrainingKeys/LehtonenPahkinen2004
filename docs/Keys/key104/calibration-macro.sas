...
%macro CalibRation(n,seed);
proc surveyselect data=Province91 out=Sample n=&n method=SRS seed=&seed noprint;
...
data Check;
set Check;
if sum(URB85)=0 or sum(URB85)=&n then put
"NOTE: PostSTRatIFICATION CHECK:"
"NOTE: PoststRatification failed because URB85 is a constant for all Sample elements."
"NOTE: Please draw a new Sample (change SEED value or increase n, the Sample size).";
run;
...
g_Reg = 1+(Pop_HOU85_Mean-HOU85_Mean)/((&n-1)/&n*HOU85_css/(&n-1))*(HOU85-HOU85_Mean);
wstar_Reg=SW*g_Reg;
