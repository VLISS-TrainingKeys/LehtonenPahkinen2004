...
for the RATIO UE91/LAB91 with their standard errors using (SUDAAN) DESCRIPT and RATIO procedures. 
...
proc descript data=Sample filetype=sas design=wor deft1;
  	...
  	setenv colwidth = 15;
  	setenv decwidth = 2;
output total setotal /
		filename=Total
        filetype=sas;
run;

proc print data=Total(keep=Total seTotal);
run;
...
proc ratio data=Sample filetype=sas design=wor;
	weight SamplingWeight;
  	nest STR;
  	totcnt N;
  	numer UE91;
...
proc print data=Ratio(keep=rhat serhat);
run;
